-- Phase 5: bind each upload attempt to one immutable server-side reservation.
...
ALTER TABLE public.material_assets
  ADD COLUMN upload_idempotency_key uuid,
  ADD CONSTRAINT material_assets_upload_idempotency_reservation_check CHECK (
    upload_idempotency_key IS NULL OR upload_reservation_id IS NOT NULL
  );

CREATE UNIQUE INDEX material_asset_upload_reservations_idempotency_key_unique
ON public.material_asset_upload_reservations (uploaded_by, upload_idempotency_key)
WHERE upload_idempotency_key IS NOT NULL;

CREATE UNIQUE INDEX material_assets_upload_idempotency_key_unique
ON public.material_assets (uploaded_by, upload_idempotency_key)
WHERE uploaded_by IS NOT NULL AND upload_idempotency_key IS NOT NULL;

DROP FUNCTION IF EXISTS public.reserve_material_asset_upload(uuid, text, text, text, bigint);
...
BEGIN
  ...
  IF FOUND THEN
    ...
    END IF;
    IF v_reservation.cancelled_at IS NOT NULL OR v_reservation.expires_at <= now() THEN
      RETURN jsonb_build_object('status', 'conflict');
    END IF;
...
DROP FUNCTION IF EXISTS public.finalize_material_asset_upload(uuid);
