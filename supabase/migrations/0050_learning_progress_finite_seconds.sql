-- Reject PostgreSQL numeric special values for learner resume positions.
-- The legacy eight-argument progress RPC remains untouched.

ALTER TABLE public.learning_progress
  DROP CONSTRAINT learning_progress_resume_seconds_check,
  ADD CONSTRAINT learning_progress_resume_seconds_check CHECK (
    resume_seconds IS NULL
    OR (
      resume_seconds >= 0
      AND resume_seconds <> 'NaN'::numeric
      AND resume_seconds <> 'Infinity'::numeric
      AND resume_seconds <> '-Infinity'::numeric
    )
  );
...
BEGIN
  ...
  IF p_product_id IS NULL
     ...
     OR (p_resume_seconds IS NOT NULL AND (
       p_resume_seconds < 0
       OR p_resume_seconds = 'NaN'::numeric
       OR p_resume_seconds = 'Infinity'::numeric
       OR p_resume_seconds = '-Infinity'::numeric
     ))
