...
type TutorFilter = "Tất cả" | "Kế toán" | "Kinh tế" | "Thống kê" | "Marketing" | "Quản trị" | "MIS" | "Luật" | "Online" | "1:1";
      ...
      result = result.filter((item) => {
        if (filter === "Kế toán") {
          return (
            ...
            item.subjects.some((s) => s.toLowerCase().includes("kế toán") || s.toLowerCase().includes("kiểm toán"))
          );
        }
        if (filter === "Kinh tế") {
          return (
        ...
        if (filter === "Thống kê") {
          return (
            ...
            item.subjects.some((s) => s.toLowerCase().includes("thống kê") || s.toLowerCase().includes("toán"))
          );
        }
        if (filter === "Marketing") {
          return (
        ...
        if (filter === "MIS") {
          return item.subjects.some(
            (s) => s.toLowerCase().includes("cơ sở dữ liệu") || s.toLowerCase().includes("hệ thống thông tin")
          );
        }
        if (filter === "Luật") {
          return (
      ...
      <div className="section-shell px-4 py-8 sm:px-8 sm:py-10 lg:px-10">
        ...
        <div className="mt-6">
          {processedTutors.length > 0 ? (
            ...
            <EmptyState onReset={handleReset} message="Không tìm thấy tutor phù hợp." />
          )}
        </div>
      </div>
    </CatalogPageShell>
