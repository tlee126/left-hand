...
export default function TutorsError({
  ...
  useEffect(() => {
    ...
    console.error("Tutors catalog route error:", error);
  }, [error]);
    ...
    <CatalogPageShell>
      <div className="section-shell flex min-h-[400px] flex-col items-center justify-center px-4 py-16 text-center sm:px-8">
        ...
        <h2 className="mt-5 text-xl font-bold text-[#1f2d5a] sm:text-2xl">
          Không thể tải danh sách Tutor
        </h2>
        <p className="mt-2 max-w-md text-sm leading-6 text-[#697598]">
          Đã xảy ra sự cố trong quá trình tải dữ liệu đội ngũ Tutor. Vui lòng thử lại hoặc liên hệ hỗ trợ nếu lỗi tiếp diễn.
        </p>
        <button
          type="button"
          onClick={() => reset()}
