...
interface SubjectWorkspaceClientProps {
  workspace: StudentWorkspaceData & { progress?: LearningProgress[]; progressUnavailable?: boolean; initialMaterialId?: string | null; requestedMaterialUnavailable?: boolean };
}
...
export function SubjectWorkspaceClient({ workspace }: SubjectWorkspaceClientProps) {
  const [activeTab, setActiveTab] = useState<TabKey>(workspace.initialMaterialId ? "documents" : "overview");
  ...
  return <div className="relative min-h-screen overflow-x-hidden bg-[#fffdf9]">
    ...
    <main className="container mx-auto px-4 py-8 md:px-8">
      ...
      <section className="rounded-[24px] border border-[#1b2e7422] bg-white p-6 shadow-sm">
        ...
        {activeTab === "documents" && (workspace.materials.length ? <div className="space-y-3"><h2 className="text-base font-extrabold text-[#132a67]">Danh mục tài liệu</h2>{workspace.materials.map((material) => { const itemId = material.productId; const key = progressKey(material.productId, "material", itemId); const retry = retryItems[key]; const completed = progressLabel(progress, material.productId, "material", itemId) === "Đã hoàn thành"; const isDirectGrant = material.accessSource === "direct_grant"; return <article id={`material-${material.productId}`} key={material.productId} className="flex flex-col gap-4 rounded-2xl border border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2 text-sm font-bold text-[#132a67]"><FileText className="h-5 w-5 text-blue-600" /><h3>{material.title}</h3><span className={`rounded-full px-2 py-1 text-[10px] font-black ${isDirectGrant ? "bg-violet-50 text-violet-700" : "bg-blue-50 text-blue-700"}`}>{isDirectGrant ? "Được cấp riêng" : "Theo quyền môn học"}</span></div><p className="mt-1 text-xs text-[#5f6d8f]">{material.description}</p><p className="mt-2 text-[11px] font-semibold text-[#8091b8]">{material.pages} trang · {progressPercent(progress, material.productId, "material", itemId)}% · {completed ? "Đã hoàn thành" : "Chưa hoàn thành"}</p></div><div className="flex flex-wrap gap-2"><MaterialViewer productId={material.productId} mimeType={material.mimeType} allowDownload={material.allowDownload} autoOpen={workspace.initialMaterialId === material.productId} /><ProgressButton pending={pendingKey === key} completed={completed} retry={Boolean(retry)} onClick={() => retry ? saveProgress(retry) : saveProgress(completedProgress(progress[key], material.productId, "material", itemId))} /></div></article>; })}</div> : <Unavailable />)}
...
export function UnavailableMaterial() {
  return <div className="relative min-h-screen overflow-x-hidden bg-[#fffdf9]"><Header /><main className="container mx-auto px-4 py-8 md:px-8"><div className="rounded-[24px] border border-amber-200 bg-amber-50 p-6"><h1 className="text-lg font-black text-[#132a67]">Tài liệu không còn khả dụng</h1><p className="mt-2 text-sm font-semibold text-amber-900">Tài liệu không còn khả dụng hoặc bạn không có quyền truy cập.</p><Link href="/ca-nhan" className="mt-4 inline-flex rounded-full bg-[#132a67] px-4 py-2 text-xs font-black text-white">Quay lại Cá nhân</Link></div></main><Footer /><FloatingActions /></div>;
}
