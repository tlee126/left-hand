...
function isValidAsset(value: unknown, productId: string): value is { productId: string; storagePath: string; mimeType: string } {
  if (value === null || typeof value !== "object" || Array.isArray(value)) return false;
  const asset = value as { productId?: unknown; storagePath?: unknown; mimeType?: unknown };
  return normalizedUuid(asset.productId) === productId
    ...
    && ["application/pdf", "video/mp4", "video/webm", "video/quicktime"].includes(asset.mimeType);
}

/** Streams the private object through the app; the provider signed URL never reaches the learner. */
