...
- The internal `/api/internal/cron/material-upload-cleanup` job runs every 15 minutes in Vercel and requires the deployment-only `CRON_SECRET` environment variable; it deletes only expired or cancelled upload-session paths.
